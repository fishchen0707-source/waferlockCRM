...
console.log('\n【4】金額處理');
{
  ...
  // ⚠ 這條規則在 2026-08-10 改過方向，理由要留著，不然下次又會被改回去。
  //
  // 原本：有空白的文字一律算 0（「不猜」）。
  // 現在：以空白切開，把每一段能解析的數字**相加**。
  //
  // 為什麼改：實際資料一列會裝兩個品項，第 168 列用 Alt+Enter、
  // 第 156 列用空格，兩格在畫面上長得一模一樣。舊規則把
  // 「56,000 16,800」黏成 5,600,016,800，某承包商合計因此顯示 112 億。
  // 「一格兩個數字」在這份試算表是常態，不是例外，必須處理。
  //
  // 代價：這確實變成在猜。「NT$ 5,000」現在會算成 5,000（這個猜是對的），
  // 但空白當千分位的寫法會被拆錯（見下一條）。取捨是：
  // 少算一格的損失有限，多算一萬倍會讓整份報表不能看。
  ok(G.num_('NT$ 5,000') === 5000, '「NT$ 5,000」取得到金額');
  ok(G.isNonNumeric_('NT$ 5,000') === true,
     '但仍要標為非數字，讓人去修那一格——能解析不等於格子填得對');
  // 已知且接受的限制：這份資料的千分位一律用逗號（「56,000」），不用空白。
  // 若哪天出現空白當千分位的寫法，這裡會拆錯，而且是安靜地錯——先記在這裡。
  ok(G.num_('1 234 567') === 802,
     '已知限制：空白當千分位會被拆開相加（本資料不用這種寫法）');
...
console.log('\n【10】簡表（日期／承包商／承包金額／承包數量／業務銷售額）');
{
  ...
  // 🔴🔴 儲存格內換行 = 一列裝兩個品項，必須相加，不可黏成一個數字。
  //     這是實機真的發生過的事：一課-eli 第 156／168 列的「發包合計」是
  //     "56,000⏎16,800"，舊 num_ 把 \s 全刪 → "5600016800"，一列變 56 億，
  //     蔣家工程行合計顯示 112 億（真實值 105 萬）。
  //     而且它長得完全像資料有問題，不像程式有問題——查了三輪才找到。
  ok(G.num_('56,000\n16,800') === 72800,
     '🔴 換行要相加成 72,800，不可黏成 5,600,016,800，實得 ' + G.num_('56,000\n16,800'));
  ok(G.num_('140\n42') === 182, '🔴 數量同理要相加成 182，實得 ' + G.num_('140\n42'));
  ok(G.num_('140\r\n42') === 182, 'Windows 換行（\\r\\n）也要處理');
  // 🔴 第二輪才發現：同一份資料裡兩種寫法並存。第 168 列用 Alt+Enter（真換行），
  //    第 156 列用空格——畫面上因為自動換行，兩格長得一模一樣。
  //    只修換行的話會出現「修好一半」，比全壞更難察覺。
  ok(G.num_('56,000 16,800') === 72800,
     '🔴 空格分隔也要相加（第 156 列就是這種），實得 ' + G.num_('56,000 16,800'));
  ok(G.num_('140 42') === 182, '空格分隔的數量同樣要相加');
  ok(G.num_('56,000　　16,800') === 72800, '全形空白也算分隔');
  ok(G.num_('140\n\n42') === 182, '空行不可讓結果變 0');
  ok(G.num_('  1,234  ') === 1234, '沒有換行時維持原本行為（去逗號與空白）');
  ok(G.num_(0) === 0 && G.num_('') === 0 && G.num_('abc') === 0, '非數字仍回 0');
  ok(G.num_('140\nabc') === 140, '一行是數字一行是文字時，取得到的那行仍要算');
  // isNonNumeric_ 要跟 num_ 講同一個故事，否則同一格會被兩個函式給出矛盾的判斷
  ok(G.isNonNumeric_('56,000\n16,800') === false, '每行都是數字就不算非數字');
  ok(G.isNonNumeric_('140\nabc') === true, '有一行不是數字就要標成非數字');

  // 端到端：真的放一列多品項進去，總額不可爆掉
  {
    reset();
    const g = BOOK['一課-eli'];
    const ml = H_E.map(() => '');
    ml[H_E.indexOf('發包申請日期')] = ser('2026-03-19');
    ml[H_E.indexOf('發包單號')] = 'EL-260319-01';
    ml[H_E.indexOf('承包商')] = '蔣家工程行';
    ml[H_E.indexOf('客戶')] = '建高工程';
    ml[H_E.indexOf('合約數量')] = '140\n42';
    ml[H_E.indexOf('發包合計')] = '56,000\n16,800';
    g.push(ml);
    const t = G.getSimple({ from: '2026-01-01', to: '2026-12-31' }).data;
    const r = t.rows.filter(x => x.orderNo === 'EL-260319-01')[0];
    ok(r && r.price === 72800,
       '🔴 一列兩品項的金額要是 72,800，實得 ' + (r && r.price));
    ok(r && r.qty === 182, '🔴 數量要是 182，實得 ' + (r && r.qty));
    ok(t.bigCount === 0,
       '🔴 正常的多品項列不可被誤判為金額異常，實得 ' + t.bigCount + ' 列');
    reset();
  }
  ...
  //    垃圾數字。只印總額等於要人回試算表翻 95 列，所以要直接把那幾列指出來。
  {
    ...
    ok(/原始內容/.test(r2),
       '🔴 警示卡要顯示原始儲存格文字——這次為了確認是換行還是空格分隔多花了一輪');
...
// 電商平台預設清單：業助新訓教材確認公司實際合作的平台不只 MOMO/蝦皮，
// 「選單」分頁還沒建好時，這些平台的出貨不該全部落到「其他」桶。
console.log('【11】電商平台預設清單');
{
  ['PCHOME', '燦坤', '特力屋', 'HOMEBOX', 'Pinkol'].forEach(function (name) {
    ok(G.DEFAULT_CHANNELS.indexOf(name) >= 0,
       '🔴 預設通路清單應含「' + name + '」（公司實際使用的電商平台）');
  });
  ok(G.DEFAULT_CHANNELS.indexOf('MOMO') >= 0 && G.DEFAULT_CHANNELS.indexOf('蝦皮') >= 0,
     '原有的 MOMO／蝦皮 不可被移除');
}
